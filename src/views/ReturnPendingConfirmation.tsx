...
export function ReturnPendingConfirmation() {
  ...
  return (
    <div className="bg-white rounded-sm border border-gray-200 animate-in fade-in duration-500 w-full h-full overflow-hidden">
          ...
          <tbody>
            {mockData.map((item, index) => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-3 border-r border-gray-200 py-2">
                  <span className="px-2 py-0.5 rounded-full bg-orange-100 text-orange-600 text-[10px]">{item.status}</span>
                </td>
      ...
      <div className="p-3 bg-white flex flex-wrap items-center justify-between gap-4 border-t border-gray-200">
        <button className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-1.5 rounded-sm text-[12px] shadow-sm transition-all focus:ring-2 focus:ring-orange-200 outline-none">
          确认订单
        </button>
      ...
      <div className="text-center text-[10px] text-gray-400 py-10">
        共执行 12 个查询，用时 0.025276 秒，Gzip 已禁用，内存占用 5.685 MB
      </div>
    </div>
  );
}
