...
import { Search, FileSpreadsheet, Info, Printer, Calendar, Send, RefreshCw } from 'lucide-react';
import { cn } from '../lib/utils';

export function ReturnOverseasPendingInWarehouse() {
  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  const mockData = [
    {
      id: 'RET-WH-001',
      customer: '义乌市速优进出口有限公司',
      refNo: 'WH-REF-77221',
      boxes: 12,
      weight: 120.4,
      warehouse: '海外中转仓-1',
  ...
  return (
    <div className="bg-white rounded-sm border border-gray-200 animate-in fade-in duration-500 w-full min-h-screen">
          ...
          <tbody>
            {mockData.map((item, index) => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-3 border-r border-gray-200 py-2">
                  <span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 text-[10px] whitespace-nowrap">{item.status}</span>
                </td>
                ...
                <td className="px-3 py-3 py-2 border-r border-gray-200">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <button className="text-gray-500 hover:text-gray-700">轨迹</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {/* Pagination Footer */}
      <div className="p-3 bg-white flex flex-wrap items-center gap-2 border-t border-gray-200">
        <select className="h-8 border border-gray-300 text-[12px] px-2 rounded-sm outline-none bg-white">
          <option>请选择海外仓</option>
        </select>
        <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-sm text-[12px] shadow-sm flex items-center gap-1">
          <Send size={14} /> 推送海外仓
        </button>
        <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-sm text-[12px] shadow-sm flex items-center gap-1">
          <RefreshCw size={14} /> 更新海外仓入库
        </button>
      ...
      <div className="text-center text-[10px] text-gray-400 py-10">
        共执行 12 个查询，用时 0.026971 秒，Gzip 已禁用，内存占用 5.695 MB
      </div>
    </div>
  );
}
