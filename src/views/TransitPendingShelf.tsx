...
export function TransitPendingShelf() {
  const MOCK_DATA = [
    { id: '1', customer: 'yueyang07', code: '12021', inboundNo: 'FX12021001', packingNo: 'FX12021001-24567', productName: '短袖', fnsku: 'D12021T8BFJ', boxes: 5, warehouse: '星卓越泰国国仓', time: '2025-12-04 13:31:42' },
    { id: '2', customer: 'yueyang01', code: '12015', inboundNo: 'FX12015001', packingNo: 'FX12015001-123', productName: '猫咪水杯', fnsku: 'D120154AGF2', boxes: 10, warehouse: '星卓越泰国国仓', time: '2025-12-04 13:30:58' },
    { id: '3', customer: 'qin', code: '12014', inboundNo: 'FX12014004', packingNo: 'FX12014004-1', productName: '猫粮10好', fnsku: 'D12014GK1AA', boxes: 2, warehouse: '星卓越泰国国仓', time: '2025-12-04 13:29:21' },
    { id: '4', customer: 'qin', code: '12014', inboundNo: 'FX12014003', packingNo: 'FX12014003-1', productName: '猫砂二号', fnsku: 'D12014TCL0Z', boxes: 20, warehouse: '星卓越泰国国仓', time: '2025-12-04 13:29:17' },
  ];

  const totalBoxes = MOCK_DATA.reduce((acc, curr) => acc + curr.boxes, 0);
  ...
  return (
    <div className="bg-[#f0f2f5] h-full overflow-hidden font-sans text-gray-700">
          ...
          <table className="w-full border-collapse text-[12px] text-left">
            <thead className="bg-[#f5f7fa] text-gray-600 font-medium">
              <tr>
                ...
                <th className="px-3 border-r border-gray-200 min-w-[80px] py-2">箱数</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {MOCK_DATA.map((item, index) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 border-r border-gray-200 font-bold py-2">{item.boxes}</td>
                  <td className="px-3 border-r border-gray-200 font-normal py-2">{item.warehouse}</td>
                  <td className="px-3 border-r border-gray-200 text-gray-500 font-normal whitespace-nowrap py-2">{item.time}</td>
                  <td className="px-3 font-normal text-orange-500 py-2 border-r border-gray-200">待上架</td>
                </tr>
              ))}
              {/* Summary Row */}
              <tr className="hover:bg-gray-50 transition-colors">
                <td colSpan={6} className="px-3 border-r border-gray-200 font-bold py-2">总计</td>
                <td className="px-3 border-r border-gray-200 font-bold py-2">{totalBoxes}</td>
      ...
      <div className="bg-white mx-2 p-2 border border-gray-200 flex flex-wrap items-center gap-2 justify-between">
        <div className="flex items-center gap-1">
          <input type="text" placeholder="货架" className="border border-gray-300 h-8 px-2 outline-none text-[12px] w-24 rounded-sm shadow-inner" />
          <button className="bg-[#ff9c3a] hover:bg-orange-600 text-white px-3 h-8 text-[12px] rounded-sm transition-colors shadow-sm font-medium">提交上架</button>
        </div>
        ...
        <div className="flex items-center gap-4 text-[11px] text-gray-500 whitespace-nowrap">
          ...
          <div className="flex items-center gap-1 border-l pl-4 border-gray-200 font-normal">
             ...
             <select className="border border-gray-300 rounded-sm px-1 h-6 ml-2 outline-none whitespace-nowrap">
                <option>1</option>
             </select>
          </div>
        </div>
      ...
      <div className="mt-8 text-center text-[11px] text-gray-400 pb-10 italic-normal">
         <p>共执行 11 个查询，用时 0.049448 秒，Gzip 已禁用，内存占用 8.820 MB</p>
      </div>
    </div>
  );
}
