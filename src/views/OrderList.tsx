    ...
    <div className="bg-white rounded-sm border border-gray-200 animate-in fade-in duration-500 w-full h-full overflow-hidden">
      ...
      <div className="p-3 bg-[#f5f7fa] flex flex-wrap gap-y-3 gap-x-4 border-b border-gray-200 sticky top-0 z-10">
        <div className="flex items-center gap-2">
          <label className="text-[12px] text-gray-600 whitespace-nowrap">参考单号</label>
        ...
        <table className="w-full text-[12px] text-left border-collapse min-w-[2000px]">
          ...
          <tbody className="divide-y divide-gray-200">
            {MOCK_DATA.map((item, index) => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-2 border-r border-gray-200">
                  <span className={cn(
                    "text-[10px]",
                    item.pushWms === '未推送' ? "text-gray-400" : "text-blue-500"
                  )}>{item.pushWms}</span>
                </td>
            ...
            <tr className="bg-[#fffdf9] font-medium text-red-500">
              <td colSpan={3} className="px-3 py-2 border-r border-gray-200">总计</td>
      ...
      <div className="p-4 flex flex-wrap items-center justify-between gap-4">
        <button className="bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded-sm text-[12px] shadow-sm transition-all active:scale-95">
          导出所选
        </button>
      ...
      <div className="text-center text-[10px] text-gray-400 py-6">
        共执行 15 个查询，用时 0.027222 秒，Gzip 已禁用，内存占用 6.746 MB
      </div>
    </div>
  );
}
