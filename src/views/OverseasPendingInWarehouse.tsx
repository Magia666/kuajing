...
import { Search, FileSpreadsheet, Info, Download, Edit, Printer, Trash2, Calendar, Archive, RotateCcw } from 'lucide-react';
...
const MOCK_DATA: OrderData[] = [
  ...
  },
  {
    ...
    status: '海外仓待入库',
    pushWms: '未推送',
    addedTime: '2026-05-10 11:20:00',
    remark: '',
    pushStatus: ''
  },
  {
    id: '3',
    customerName: 'shanghai_biz',
    customerCode: '13011',
    refNo: 'FX13011001',
    totalBoxes: 5,
    inBoxes: 0,
    ...
    status: '海外仓待入库',
    pushWms: '已推送',
    addedTime: '2026-05-11 08:30:00',
    remark: '打板出货',
    pushStatus: ''
  }
];

export function OverseasPendingInWarehouse() {
  ...
  return (
    <div className="bg-white rounded-sm border border-gray-200 animate-in fade-in duration-500 w-full h-full overflow-hidden">
              ...
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-3 py-2 border-r border-gray-200">
                  <div className="flex flex-wrap gap-2 text-blue-500">
                    ...
                    <button className="hover:underline flex items-center gap-0.5"><Archive size={12}/>[海外入库]</button>
                    <button className="hover:underline flex items-center gap-0.5"><Printer size={12}/>[打印]</button>
                    <button className="hover:underline flex items-center gap-0.5 text-red-500"><RotateCcw size={12}/>[回退]</button>
      ...
      <div className="text-center text-[10px] text-gray-400 py-6">
        共执行 15 个查询，用时 0.021030 秒，Gzip 已禁用，内存占用 6.749 MB
      </div>
    </div>
  );
}
